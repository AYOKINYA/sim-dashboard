            ...
            .then(
                (res) => {
                    ...
                    res.data.forEach((item, idx) => {
                        if (idx === 0) {
                            xLabels.push("실적");
                            yValues.push(item.SetBoxBigBoxSimulationCount); // 원래는 RealCount! 지금은 데이터 때문에..
                        }
                        xLabels.push('#' + item.SimulationNumber);
                        yValues.push(item.SetBoxBigBoxRealCount); // 원래는 SimulationCount!
                    })
...
export default OverallBigSetBox;